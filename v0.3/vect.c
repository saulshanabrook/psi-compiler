...
/*
    ...
    if (static_value(vect)) {
      if (vect->ident->array.shp[0]->type==FLOAT)
	if (vect->ident->array.shp[0]->value==1.0) {
	  vect->ident->type=FLOAT;
	  vect->ident->value=vect->ident->array.cnst[0];
	}
      return(vect);
    }
*/
...

{
  ...
  if (ident->type==CONST_ARRAY) {
    ...
    fprintf(vfile,"  double const%d[]={",const_num);
