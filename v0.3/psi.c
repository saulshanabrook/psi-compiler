...

{
  ...
  print_rav(tfile,expr->right->shp,0);
...

{
  ...
  if (inright) {
    ...
    assign(expr,result,TRUE,op,FALSE);
  }

  fa_reduced->d.list=reduced;
  reduced=fa_reduced;
  ...
  case MINUS:
    ...
    if (inright&&((*new_op[0]!='+')||(op[0]!='+'))) {
      ...
      *tmp->index=zero_vec;
      ...
      tmp->ident=get_ident();
      sprintf(tmp->ident->string,"temp%d",temp_array_num);
      if (result->ident->type==VAR_FLOAT) tmp->ident->type=VAR_FLOAT;
...
*/
    ...
    tmp2->bound=vect_cat(&one_vec,expr->bound);
...

{
  ...
  /* remove and assignments of the type A=A, at the same to move
  ...
  node=reduced;
  while (node!=NULL) {
    if ((node->type==NOP)&&(node->d.frag->ident==result->ident)) {
...
psi_reduce(statement_t *statement)
