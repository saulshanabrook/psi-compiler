...
int bFindUniqueIdentifiers(s_expr_t *pseNode) {
  ...
  TailRecursion:
    ...
    if (pseNode == NULL) {
        error ("FATAL ERROR: Scaler.c (#0), A unexpected NULL was encountered in an s_expr_t.");
	return(FALSE);
	} /* if (pseNode == NULL) */
...
FractionalPolygon_t *pfpSExprToFractionalPolygon(s_expr_t *pseNode) {
  ...
  int index;
    ...
    if (pseNode == NULL) {
	error ("FATAL ERROR: Scaler.c (#2), unexpected NULL found in s_expr_t");
	return (NULL);
	}
	...
	default :
	    report ("WARNING: Scaler.c (#3), an unexpected s_expr_t op was encountered.");
