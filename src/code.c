...
/* this makes the second if in do_shapes unnecessary.  should change later */
  ...
  if (expr==NULL)  {q1("hm");return;}  if (expr->loc==NULL) {
    dotab(0);
    fprintf(tfile,"shift=%s;\n",result->ident->string);
  } else {
    sprintf(buf,"shift=%s+",result->ident->string);
     {q1("hm");return(my_gamma(tfile,buf,expr->loc,result->shp,result->dim));}  }
  expr_free(temp_res);
}
...

{
  ...
  if (expr==NULL)  {q1("hm");return;}  if (expr->index==NULL) {
    dotab(0);
    fprintf(tfile,"offset%d=%s;\n",num,expr->ident->string);
  } else {
    sprintf(buf,"offset%d=%s+",num,expr->ident->string);
     {q1("hm");return(my_gamma(tfile,buf,expr->index,expr->shp,expr->dim));}  }
}
