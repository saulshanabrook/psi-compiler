...
q1("In empty, see if it is an empty array.");
  if (vect->op!=NOP)		{
q1("Yes, it is an empty expression.");
    return(FALSE);
				}

  /* check to see if elements of the shape are 0 */
  q1("See if (vect->ident->type==CONST_ARRAY)");
  if (vect->ident->type==CONST_ARRAY) {
  q1("Yes, (vect->ident->type==CONST_ARRAY)");
qident("vect->ident",vect->ident);
    for (i=0; i<vect->ident->array.shp[0]->value; i++)	{
    q1("for (i=0; i<vect->ident->array.shp[0]->value; i++)");
