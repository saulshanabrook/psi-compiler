...

{
  ...
  allocate A2 b;
  forall (<0> <= t < <i>) {
    A2=((b ptake (<0> drop A))+(b ptake (<2> drop A)))*m_lambda;
    <1> pdrop A=(b ptake (<1> drop A))*lambda+A2;
  }
}
...

{
  ...
  forall (<0> <= t < <i>) {
	...
	(b ptake (<2 1> drop A))+(b ptake (<0 1> drop A)))*m_lambda;
    <1 1> pdrop A=(b ptake (<1 1> drop A))*lambda+A2;
  }
}
...

{
  ...
  forall (<0> <= t < <i>) {
	...
	(b ptake (<2 1 1> drop A))+(b ptake (<0 1 1> drop A)))*m_lambda;
    <1 1> pdrop A=(b ptake (<1 1 1> drop A))*lambda+A2;
  }
}
