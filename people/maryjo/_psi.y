...
%type <statement> alloc_statement
%type <statement> actual_list_base
%type <reduced> actual_list_opt
...
actual_list_base:
	NAME1
	{ $<reduced>$ = actual_list_1($1); }
	actual_list_opt
	{ $$=actual_list_2($<reduced>2,$3); }
	;

actual_list_opt: { $$ = NULL; }
	| ',' NAME1
	{ $<reduced>$=actual_list_3($2); }
	actual_list_opt
	{ $$ = actual_list_4($<reduced>3,$4); }
	;

procedure_call:
	NAME1 '('
	{ do_procedure_call_1 ($1); }
	actual_list_base ')' ';'
	/* The folloing function doesn't do anything yet -- thom */
	{ $$ = do_procedure_call_2 ($4); }
	;

alloc_statement:
	ALLOCATE1  NAME1 expression  ';' { $$ = allocate_statement($2,$3); }
	;
...
/* unary operators */
	...
	| omega_unop expression %prec OMEGAU
		{ $$ = omega_unary ($1, $2); }
	| expression omega_binop expression %prec OMEGAB
		{ $$ = omega_binary ($1, $2, $3); }
	;
...
omega_unop :
		unary_op OMEGA1 '<' vector_const_ident_base '>'
		{ $$ = omega_no_bracket ($1, $4); }
	|	'[' omega_unop ']' OMEGA1 '<' vector_const_ident_base '>'
		{ $$ = omega_yes_bracket ($2, $6); }
		;

omega_binop :
		binary_op OMEGA1 '<' vector_const_ident_base '>'
		{ $$ = omega_no_bracket ($1, $4); }
	|	'[' omega_binop ']' OMEGA1 '<' vector_const_ident_base '>'
		{ $$ = omega_yes_bracket ($2, $6); }
		;
