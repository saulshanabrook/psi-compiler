...
/* psi_sect: creates a parser_t for arg1 subscripted by #s in array. */
/*              note: returns pointer to arg1 - no new node created.   */
parser_t *psi_sect(parser_t *arg1,int start[],int stop[],int stride[],int dim){
  ident_t *tident;
  vect_t *tvect;
  expr_t *array = arg1->psi;
  ident_t *peek = array->ident;
  vect_t *tstride = array->stride;
  vect_t *tbound = array->bound;
  vect_t *tindex = array->index;
  int i, components[7];

  /* compute the # of components in each dimension */

  for (i=0; i< dim; i++)
    components[i] = ((stop[i] - start[i])/stride[i]) + 1;

  /* allocate new shtuff for shape -- not at all confident about this */

  if (array->shp==NULL) {
    array->shp=get_vect();
    array->shp->shp=get_s_expr();
    array->shp->shp->ident=get_ident();
    array->shp->shp->ident->type=CONSTANT;
    array->shp->shp->ident->value=dim;
    array->shp->shp->ident->array.dim=&const1;
    array->shp->shp->ident->array.shp=(ident_t **) get_mem(sizeof(ident_t *));
    array->shp->shp->ident->array.shp[0]=&const1;
    array->shp->ident=get_ident();
    array->shp->ident->type=RAV;
    array->shp->ident->array.dim->value=dim;
    array->shp->ident->array.rav=(s_expr_t **) 
      get_mem(dim*sizeof(s_expr_t *));
  }

/* new shape or old, i gotta change it here. */

  for (i=0; i<dim; i++) {
    array->shp->ident->array.rav[i]=get_s_expr();
    array->shp->ident->array.rav[i]->ident=mk_id_cnst(components[i]);
  }

  if (tindex == NULL) {
    tindex=get_vect();
    tindex->ident=get_ident();
    tindex->ident->type=CONST_ARRAY;
    tindex->ident->array.cnst = (double *) get_mem(dim * sizeof(double));
    for (i=0; i<dim; i++) tindex->ident->array.cnst[i]=start[i]-1.0;
    array->index = tindex;
  }
  else {
    assert(tindex->ident->type==CONST_ARRAY);
    assert(tstride != NULL);
    assert(tstride->ident->type==CONST_ARRAY);
    for (i=0; i<dim; i++) 
      tindex->ident->array.cnst[i] +=
	(start[i]-1.0)*tstride->ident->array.cnst[i];
  }

  if (tstride == NULL) {
    tstride=get_vect();
    tstride->ident=get_ident();
    tstride->ident->type-CONST_ARRAY;
    tstride->ident->array.cnst = (double *) get_mem(dim * sizeof(double));
    for (i=0; i<dim; i++) tstride->ident->array.cnst[i]=(double)stride[i];
    array->stride=tstride;
    /* newstride[i] = stride[i];*/
  }
  else {
    /* newstride[i] = oldstride[i] * stride[i];*/
    assert(tstride->ident->type==CONST_ARRAY);
    for (i=0; i<dim; i++) 
      tstride->ident->array.cnst[i] *= stride[i];
  }

#if 0
  if (tbound == NULL) {
    tbound=get_vect();
    tbound->ident=get_ident();
    tbound->ident->type=CONST_ARRAY;
    tbound->ident->array.cnst = (double *) get_mem(dim * sizeof(double));
    for (i=0; i<dim; i++)
      tbound->ident->array.cnst[i]=(components[i])?(double)(components[i]):1.0;
    array->bound = tbound;
